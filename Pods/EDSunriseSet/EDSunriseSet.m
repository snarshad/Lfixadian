...
static NSCalendar *s_gregorian = nil;

@interface EDSunriseSet()
@property (nonatomic, strong) NSCalendar *calendar;
@end

@interface NSDate (floor)
- (NSDate *)dateFloor;
@end

@implementation NSDate (floor)
- (NSDate *)dateFloor
{
    ...
    [s_gregorian setTimeZone:[NSTimeZone localTimeZone]];
    
    @synchronized (s_gregorian)
    {
        components = [s_gregorian components:unitFlags fromDate:self];
    }
    components.hour = 0;
    components.minute = 0;
    ...
    @synchronized (s_gregorian)
    {
        midnight = [s_gregorian dateFromComponents:components];
    }
...
#pragma mark -
@interface EDSunriseSet ()
@property (nonatomic, readwrite) NSTimeInterval sunsetSeconds;
@property (nonatomic, readwrite) NSTimeInterval sunriseSeconds;
@end
...
@synthesize sunsetSeconds, sunriseSeconds;

static const int secondsInHour= 60.0*60.0;

#pragma mark - Shared Manager
static EDSunriseSet *s_sharedManager = nil;

+ (EDSunriseSet *)sharedManager {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        s_sharedManager = [[EDSunriseSet alloc] init];
    });

    return s_sharedManager;
}

+ (void)setSharedTimezone:(NSTimeZone *)timezone latitude:(double)latitude longitude:(double)longitude {
    [self sharedManager].latitude = latitude;
    [self sharedManager].longitude = longitude;

    [self sharedManager].timezone = timezone;
    
    [[self sharedManager] calculate:[NSDate date]];
}
...
- (EDSunriseSet *)initWithTimezone:(NSTimeZone *)tz latitude:(double)lat longitude:(double)longt {
    ...
    if( self )
    {
        ...
        s_gregorian = [[NSCalendar alloc] initWithCalendarIdentifier:NSGregorianCalendar];
...
- (instancetype)init {
    if (self = [super init]) {
        ...
        [self calculate:[NSDate date]];
        
        //Note - still need latitude, longitude, and timezone set to be useful
    }
    return self;
}
...
{
    // Get date components 
    [s_gregorian setTimeZone:_timezone];
    NSDateComponents *dateComponents = [s_gregorian components:( NSYearCalendarUnit | NSMonthCalendarUnit |  NSDayCalendarUnit ) fromDate:date];
    ...
    [self sunRiseSetForYear:(int)[dateComponents year] month:(int)[dateComponents month] day:(int)[dateComponents day] longitude:_longitude latitude:_latitude
                      trise:&rise tset:&set ];
    self.sunrise = [self utcTime:dateComponents withOffset:(NSTimeInterval)(rise*secondsInHour)];
    self.sunset  = [self utcTime:dateComponents withOffset:(NSTimeInterval)(set*secondsInHour)];
    
    self.sunriseSeconds = rise * secondsInHour;
    self.sunsetSeconds = set * secondsInHour;
}
...
-(NSDate*)utcTime:(NSDateComponents*)dateComponents withOffset:(NSTimeInterval)interval
{
    [s_gregorian setTimeZone:_utcTimeZone];
    return [[s_gregorian dateFromComponents:dateComponents] dateByAddingTimeInterval:(NSTimeInterval)(interval)];
}
...
-(NSDateComponents*)localTime:(NSDate*)refDate
{
    ...
    NSDateComponents *dc = [s_gregorian components:( NSHourCalendarUnit  | NSMinuteCalendarUnit | NSSecondCalendarUnit) fromDate:refDate] ;
